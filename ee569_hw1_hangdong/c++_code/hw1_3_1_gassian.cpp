...
//start build up denoise
    ...
    int m1=1,m2=4,m3=17,mm=4*m1+4*m2+m3;
    for (int i = 0; i < Size_r; i++) {
        for (int j = 0; j < Size_c; j++) {
            int value[3] = {};
            value[0]=m1*Imagedataresize[i][j][0]+m2*Imagedataresize[i][j+1][0]+m1*Imagedataresize[i][j+2][0]+m2*Imagedataresize[i+1][j][0]+
                    m3*Imagedataresize[i+1][j+1][0]+m2*Imagedataresize[i+1][j+2][0]+m1*Imagedataresize[i+2][j][0]+m2*Imagedataresize[i+2][j+1][0]+
                    m1*Imagedataresize[i+2][j+2][0];
            value[1]=m1*Imagedataresize[i][j][1]+m2*Imagedataresize[i][j+1][1]+m1*Imagedataresize[i][j+2][1]+m2*Imagedataresize[i+1][j][1]+
                     m3*Imagedataresize[i+1][j+1][1]+m2*Imagedataresize[i+1][j+2][1]+m1*Imagedataresize[i+2][j][1]+m2*Imagedataresize[i+2][j+1][1]+
                    m1*Imagedataresize[i+2][j+2][1];
            value[2]=m1*Imagedataresize[i][j][2]+m2*Imagedataresize[i][j+1][2]+m1*Imagedataresize[i][j+2][2]+m2*Imagedataresize[i+1][j][2]+
                     m3*Imagedataresize[i+1][j+1][2]+m2*Imagedataresize[i+1][j+2][2]+m1*Imagedataresize[i+2][j][2]+m2*Imagedataresize[i+2][j+1][2]+
                    m1*Imagedataresize[i+2][j+2][2];
            //int pixel = floor(matrix_size / 2);
            output[i][j][0] = value[0] / mm;
            output[i][j][1] = value[1] / mm;
            output[i][j][2] = value[2] / mm;
        }
    }
    /////////////////////////FINISH MY CODE////////////////////////////////////////////
