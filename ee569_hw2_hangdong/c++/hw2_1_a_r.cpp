...

{
    ...
    fread(Warp, sizeof(unsigned char), size_r*size_c*BytesPerPixel, file);
    fclose(file);
    ...
    for (int i = 0; i < size_r / 2; i++) {
        ...
        for(int j = 0; j <N; j++) {
            for (int k = 0; k < 3; k++) {
                u = ((float)M/(float)N)*(float)j;
                v = u - floor(u);
                y = floor(u);
                Imagedata[i][j][k]=(1-v)*Warp[i][(N-M+y)][k]+(v)*Warp[i][(N-M+y)+1][k];
            }
        }
    }
    // For part 2
    for (int i = 0; i < size_r / 2; i++) {
        M=pow((N*N-(N-i-1)*(N-i-1)),0.5);
        for (int j = size_c/2; j < size_c; j++) {
            for (int k = 0; k < 3; k++) {
                u = ((float)M/(float)N)*(float)(j- N)+N;
                v = u - floor(u);
                y = floor(u);
                Imagedata[i][j][k]=(1-v)*Warp[i][(y)][k]+(v)*Warp[i][(y)+1][k];
            }
        }
    }
    // For part 3
    for (int i = size_r/2; i < size_r; i++) {
        M=pow((N*N-(N-i-1)*(N-i-1)),0.5);
        for (int j = 0; j < size_r/2; j++) {
            for (int k = 0; k < 3; k++) {
                u= ((float)M/(float)(N)*(float)j)+N-M;
                v = u - floor(u);
                y = floor(u);
                Imagedata[i][j][k]=(1-v)*Warp[i][(y)][k]+(v)*Warp[i][(y)+1][k];
            }
