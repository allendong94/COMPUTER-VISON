        ...
        if(count%2==1)
        {
            for (int j = 0; j < size_c; j++) {
                for (int k = 0; k < BytesPerPixel; k++) {
                    ...
                    //Floyd method
                    if (j+1<size_c)
                    {te_err[i][j+1][0]=te_err[i][j+1][0]+error*7/16;}
                    if (j-1>=0&&(i+1)<size_r)
                    {te_err[i+1][j-1][0]=te_err[i+1][j-1][0]+error*3/16;}
                    if (i+1<size_r)
                    {te_err[i+1][j][0]=te_err[i+1][j][0]+error*5/16;}
                    if (i+1<size_r&&(j+1)<size_c)
                    {te_err[i+1][j+1][0]=te_err[i+1][j+1][0]+error*1/16;}
                }
            }
        }
        else
