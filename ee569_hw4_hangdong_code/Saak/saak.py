...
def classify_rf(train_feature, train_label, test_feature, test_label):
    #assert train_feature.shape[1] == test_feature.shape[1]
    #assert train_feature.shape[0] == train_label.shape[0]
    #assert test_feature.shape[0] == test_label.shape[0]
    rf = RandomForestClassifier()
    rf.fit(train_feature, train_label)
    accuracy = rf.score(test_feature, test_label)
    return accuracy
...
if __name__=='__main__':
    ...
    num_test = 10000
    test_data, test_labels = create_numpy_dataset(num_test, test_loader)
    ...
    accuracy = classify_svm(final_feat, labels, test_final_feat, test_labels)

    print("Accuracy: %.3f" % accuracy)

    # fit random forest classifier
    print("\nDo classification using RF")
    accuracy = classify_rf(final_feat, labels, test_final_feat, test_labels)
    print("Accuracy: %.3f" % accuracy)
